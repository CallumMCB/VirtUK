...
msoa_lookup_output_fp = f'{paths.data_path}/input/geography/msoa_lookup.csv'
...
# Note: We now also include 'msoa21nm' (renamed to 'msoa_name') for the MSOA lookup.
df_area_msoa = df_filtered.loc[:, ['oa21cd', 'msoa21cd', 'msoa21nm', 'ladcd', 'ladnm']].rename(
    columns={
        'oa21cd': 'area',
        'msoa21cd': 'msoa',
        'msoa21nm': 'msoa_name',
...
# Function to assign lad_code, lad and also msoa_name based on a matching msoa
def assign_lad_and_msoa_info(row, reference_df):
    matching_row = reference_df[reference_df['msoa'] == row['msoa']].iloc[0]
    row['lad_code'] = matching_row['lad_code']
    row['lad'] = matching_row['lad']
    row['msoa_name'] = matching_row['msoa_name']
    return row

missing_output_areas_df = missing_output_areas_df.apply(assign_lad_and_msoa_info, reference_df=df_area_msoa, axis=1)
...
# *** Modification: Remove the LAD name and MSOA name from the main file ***
# Keep only the lad_code and msoa (code) in the main file.
df_area_msoa_lad_region = df_area_msoa_lad_region.drop(columns=['lad', 'msoa_name'])

# Create a separate lookup file for LAD code <-> LAD name mapping.
df_lad_lookup = df_area_msoa[['lad_code', 'lad']].drop_duplicates()
df_lad_lookup.sort_values(by='lad_code', inplace=True)

# Create a separate lookup file for MSOA code <-> MSOA name mapping.
df_msoa_lookup = df_area_msoa[['msoa', 'msoa_name']].drop_duplicates()
df_msoa_lookup.sort_values(by='msoa', inplace=True)
