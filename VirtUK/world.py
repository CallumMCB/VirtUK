...
from VirtUK.demography import Demography, Population
...
from VirtUK.geography import Geography, Areas
from VirtUK.groups import Supergroup, Cemeteries
...
def _populate_areas(areas: Areas, demography, ethnicity=True, comorbidity=False):
    logger.info("Populating areas")
    people = Population()
    # area_list = areas.Area if hasattr(areas, 'Area') else list(areas)
    # print(area_list)
    for area in tqdm(areas, desc="Populating Areas", unit="area"):
...
class World:
    ...
    def to_hdf5(self, file_path: str, chunk_size=100000):
        """
        Saves the world to a hdf5 file. All supergroups and geography
        ...
        """
        from VirtUK.hdf5_savers import save_world_to_hdf5
